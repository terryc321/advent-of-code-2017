...
(define radius 100.0d0)

(define (deg->rad n) (* n (/ 3.1415926535898d0 180)))
...
east = (* R 3/2)
north-east = east + vert
south-east = east - vert


|#


(define horz (/ radius 2))
(define vert (* radius (sqrt 3) 1/2))

(define (north-east p)
  (let ((x (car p))
	(y (cadr p)))
    (set! x (+ x (* radius 3/2)))
    (set! y (+ y (* radius (sqrt 3) 1/2)))
    (list x y)))
...
(define (iter p xs)
  (cond
   ...
   (#t (let ((dir (car xs)))
	 (cond
	  ((eq? dir 'ne) (iter (north-east p) (cdr xs)))
	  ((eq? dir 'nw) (iter (north-west p) (cdr xs)))
	  ((eq? dir 'n) (iter (north p) (cdr xs)))
	  ((eq? dir 's) (iter (south p) (cdr xs)))
	  ((eq? dir 'se) (iter (south-east p) (cdr xs)))
	  ((eq? dir 'sw) (iter (south-west p) (cdr xs)))
	  (#t (error (format #f "unknown direction ~a" dir))))))))


(define (target)
  (let ((initial-position '(0.0d0 0.0d0)))
    (iter initial-position input)))


(define (square x)
  (* x x))

(define (distance-between pos pos2)
  (let ((x1 (car pos))
	(y1 (cadr pos))
	(x2 (car pos2))
	(y2 (cadr pos2)))    
...
(define (close-enough? pos pos2)
    (let ((dist (distance-between pos pos2)))
      ;;(format #t "distance ~a ~%" dist)
      (< dist (/ radius 5))))

(define expected-distance
  (let* ((tar (target))
	 (x (car tar))
	 (y (cadr tar)))
...
(define (search initial-position target)
  (let ((steps 0)
	(path '())
	(best-so-far 9999999999999)
	(best-so-far-path '()))
    
    (call/cc (lambda (exit)
	       (letrec ((search-recur (lambda (pos tar n path exit)
					;;(format #t "steps ~a ~%" n)
					(cond
					 ((close-enough? pos tar)
					  (cond
					   ((< n best-so-far)
					    (set! best-so-far n)
					    (set! best-so-far-path path)
					    (format #t "found next best path : dist ~a ~%" n)
					    (exit n)
					    )))
					 (#t
					  (cond
					   ((> n best-so-far) #f)
					   (#t ;; sort based on which brings closer to target

					    (let ((options
						   (sort 
						    (map (lambda (x) (let* ((fn (car x))
									    (dir (cadr x))
									    (end (fn pos))
									    (dist (distance-between end tar)))
								       (list dist end dir)))
							 direction-list)
						    (lambda (x y)
						      (< (car x) (car y))))))
					      (for-each (lambda (x)
							  (search-recur (cadr x) tar (+ n 1) (cons (caddr x) path) exit))
							options)))))))))
		 (search-recur initial-position target steps path exit))))))



#|

does computation terminate ? 

if we add an exit , terminate search does this matter ?
is there a solution we miss by not waiting further ?

scheme@(guile-user)> (search (list 0 0) (target))
found next best path : dist 720 
$67 = 720
scheme@(guile-user)> 

turn list of directions into a list of positions

;;(map (lambda (fn) (fn (list 0 0))) (list north south north-east north-west south-east south-west))
...
(define list-of-positions
  (letrec ((iter2
	    (lambda (p n xs ys)
	      (format #t "p = ~a : step ~a ~%" p n)
	      (cond
	       ((null? xs) (cons p ys))
	       (#t (let ((dir (car xs))
			 (zs (cons p ys))
			 (m (+ n 1)))
		     (format #t "direction dir ~a ~%" dir)
		     (cond
		      ((eq? dir 'ne) (iter2 (north-east p) m (cdr xs) zs))
		      ((eq? dir 'nw) (iter2 (north-west p) m (cdr xs) zs))
		      ((eq? dir 'n)  (iter2 (north p)      m (cdr xs) zs))
		      ((eq? dir 's)  (iter2 (south p)      m (cdr xs) zs))
		      ((eq? dir 'se) (iter2 (south-east p) m (cdr xs) zs))
		      ((eq? dir 'sw) (iter2 (south-west p) m (cdr xs) zs))
		      (#t (error (format #f "unknown direction ~a" dir))))))))))
  (let ((s '(0 0))
	(n 0)
	(acc '()))
...
$11 = (1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1485 1484 1484 1484 1484 1484 1484 1484 1484 1484 1484 1483 1483 1483 1483 1483 1483 1483 1483 1483 1483 1483 1483 1483 1483 1483 1483 1483 1483 1482 1482 1482 1482 148

suggests furthest is 1485
