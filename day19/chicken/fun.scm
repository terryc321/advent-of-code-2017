...
(define (iterate g x y direction letters result)
  
  (define (go-right)
    (format #t "going right ~%")
    (iterate g (+ x 1) y 'right letters result))
  (define (go-left)
    (format #t "going left ~%")
    (iterate g (- x 1) y 'left letters result))
  (define (go-up)
    (format #t "going up ~%")
    (iterate g x (- y 1) 'up letters result))
  (define (go-down)
    (format #t "going down ~%")
    (iterate g x (+ y 1) 'down letters result))
  (define (found-letter ch)
    (format #t "found letter ~a ~%" ch)
    (cond
     ((not (member ch letters))
      (iterate g x y direction (cons ch letters) result))))
...
(define (puzzle g)
  (let* ((start (find-start-square g))
    ...
    (call/cc (lambda (result)
	       (iterate g x y direction letters result)))))
...
...
going left 
iterate position 1 4 with direction left and letters found so far (F E D C B A) 
elem [#f] at grid [1] [4] 
