...
(define group-total 0)
(define group-height 0)
(define non-cancelled-chars 0)
...
(define (advance)
  ...
  (cond
   ((and (>= i 0) (< i vlen))
    (when verbose (format #t "advanced char = [~a]~%" (vector-ref arr i)))
    #t
    )
   (#t
    (when verbose (format #t "end of input reached (~a) ~%" i))
    #t
    )
   ))
...
(define (garbage-recur)
  (cond
   ((is-char #\!)
    ...
    (garbage-recur))
   ((is-char #\>)
    (when verbose (format #t "garbage end at ~a ~%" i))
    (advance)
    #t)
   (#t (advance)
       (set! non-cancelled-chars (+ 1 non-cancelled-chars))
       (garbage-recur))))
...
(define (garbage)
  (cond
   ((is-char #\<)
    (when verbose (format #t "garbage identified start ~a~%" i))
    (advance)
    (garbage-recur))))
...
(define (group-recur)
  (cond
   ((is-char #\{)
    (set! group-height (+ 1 group-height))
    (set! group-total (+ group-height group-total))
    (when verbose (format #t "group recognised start at ~a [~a] ~%" i (vector-ref arr i)))
    (advance)
    ...
    (letrec ((foo (lambda ()
		    (cond
		     ((is-char #\}) (advance)
		      (set! group-height (+ -1 group-height))
		      (when verbose (format #t "end of group at ~a ~%" (- i 1)))
		      #t)
		     (#t
		      (cond
		       ((is-char #\<) (garbage))
...
(define (run a)
  (reset)
  (set! arr a)
  (when verbose (format #t "checking ~a ~%" arr))
  (set! i 0)
  (set! vlen (vector-length arr))

  (when verbose (format #t "vlen = ~a ~%" vlen))
  (group-recur)
  (when verbose (format #t "group total ~a ~%" group-total))
  (when verbose (format #t "non cancelled chars ~a ~%" non-cancelled-chars))
  (values group-total non-cancelled-chars)
  )
...
(define (test)
  (map (lambda (x)
	 (let ((in (car x))
	       (expect (cadr x))
	       (result (call-with-values (lambda () (run (list->array 1 (string->list (car x)))))
			 (lambda (gp nc) gp))))
	   (cond
	    ((equal? expect result) (list 'PASS in result))
