...
what is the shortest delay so do not get caught - ie cost of zero
delay being run (map (lambda (f) (f 'inc)) state)) each delay ...

|#

(define (shortest-model state)
  (let ((total-cost 0))
    (do-for (depth 1 (+ 1 (length state)))
...
(define (shortest-delay scanner)
  (call/cc (lambda (exit)
	     (let ((delay 0))
	       (do-while #t
			 (let ((state (encode-scanner scanner)))
			   (do-times (delay delay)
				     (map (lambda (f) (f 'inc)) state))
			   (let ((cost (shortest-model state)))
			     (format #t "\rdelay ~a : cost ~a           " delay cost)
			     (cond
			      ((zero? cost) (exit delay)))))
			 (set! delay (+ 1 delay)))))))


(define (part-2)
  (let ((result (shortest-delay input)))
