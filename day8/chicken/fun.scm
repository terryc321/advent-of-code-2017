...
vector-ref 0 to 999 inclusive
|#
(define (collect-vars p)
  (let ((vars '()))
  (do-for i (0 1000)
	  (let ((cmd (vector-ref p i)))
	    (match cmd
	      ((v i n 'if v2 op n2)
	       ;; (format #t "i = ~a ~%" i)
	       ;; (format #t "v ~a : i ~a : n ~a : v2 ~a : op ~a : n2 ~a ~%"
	       ;; 	       v i n v2 op n2)
	       (when (not (member v vars)) (set! vars (cons v vars)))
	       (when (not (member v2 vars)) (set! vars (cons v2 vars)))       
		
	       ))))
  ;; with vars produce defines
  (do-list (v vars)
	   (format #t "(define ~a 0)~%" v))))
...
(define (encode p)
  (do-for i (0 1000)
	  (let ((cmd (vector-ref p i)))
	    (match cmd
	      ((v i n 'if v2 op n2)
	       (format #t "(when (~a ~a ~a) " op v2 n2)
	       (cond
		((eq? i 'inc)
		 (format #t "(set! ~a (+ ~a ~a))" v v n))
		((eq? i 'dec)
		 (format #t "(set! ~a (- ~a ~a))" v v n)))

	       (format #t "(when (> ~a *max*) (set! *max* ~a))" v v)	       
	       (format #t ")~%")
